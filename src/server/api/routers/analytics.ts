...
export const analyticsRouter = createTRPCRouter({
  ...
  getDashboardStats: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      const userGrowth = ((currentUsers - lastMonthUsers) / lastMonthUsers * 100)

      // Simulate conversion rate
      const conversionRate = Math.random() * 5 + 2 // 2-7%
      const lastMonthConversion = Math.random() * 5 + 2
      const conversionGrowth = ((conversionRate - lastMonthConversion) / lastMonthConversion * 100)
  ...
  getRevenueChart: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        const revenue = Math.floor(baseRevenue + trendBonus + Math.random() * 100)
  ...
  getUserAnalytics: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      for (let i = days - 1; i >= 0; i--) {
        ...
        const registrationsForDay = userRegistrations.filter((reg: any) => 
          reg.createdAt.toISOString().split('T')[0] === dateStr
        )
        
        const count = registrationsForDay.reduce((sum: number, reg: any) => sum + reg._count.id, 0)
  ...
  getConversionFunnel: protectedProcedure.query(async ({ ctx }) => {
    try {
      ...
      return [
        ...
        { stage: 'Sign-ups', count: signups, percentage: Math.round(signups / visitors * 100) },
        { stage: 'Activations', count: activations, percentage: Math.round(activations / visitors * 100) },
        { stage: 'Subscriptions', count: subscriptions, percentage: Math.round(subscriptions / visitors * 100) },
      ]
    } catch (error) {
      throw new TRPCError({
        code: 'INTERNAL_SERVER_ERROR',
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const activities = recentUsers.map((user: any) => ({
          ...
          theme: ['neon', 'electric', 'cyber', 'aurora', 'plasma'][Math.floor(Math.random() * 5)],
        }))
