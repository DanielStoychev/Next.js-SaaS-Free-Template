...
import { 
  UserPlus, Mail, Send, X, Shield, Users, Crown, Eye 
} from 'lucide-react'
...
export function InviteMemberDialog({ organizationId, onSuccess }: InviteMemberDialogProps) {
  ...
  const availableRoles = roleDetails?.roles.filter(role => role.id !== 'OWNER') || []
      ...
      <DialogContent className="sm:max-w-lg bg-gradient-to-br from-gray-900/95 to-gray-800/95 border-gray-800">
        <DialogHeader>
          ...
          <DialogDescription className="text-gray-400">
            Send an invitation to join your team with the specified role and permissions
          </DialogDescription>
        </DialogHeader>
            ...
            <Card className="bg-gray-900/30 border-gray-800">
              <CardHeader className="pb-3">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${ROLE_COLORS[role as keyof typeof ROLE_COLORS]}`}>
                  ...
                  <div>
                    <CardTitle className="text-sm text-white">
                      {availableRoles.find(r => r.id === role)?.name} Permissions
                    </CardTitle>
              ...
              <CardContent className="pt-0">
                <div className="flex flex-wrap gap-1">
                  ...
                  {(availableRoles.find(r => r.id === role)?.permissions.length || 0) > 6 && (
                    <Badge variant="outline" className="text-xs border-gray-600 text-gray-400">
                      +{(availableRoles.find(r => r.id === role)?.permissions.length || 0) - 6} more
                    </Badge>
                  )}
                </div>
              </CardContent>
